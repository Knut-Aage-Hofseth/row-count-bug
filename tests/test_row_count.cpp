...
// ---------------------------------------------------------------------------
// Phase 2: small, committed, purely-synthetic fixtures (generated by
// tests/fixtures/generate_test_files.py, buf_size=64) — no real data
// involved, always run (never skipped). "boundary" engineers a CRLF split
// exactly across the buf_size boundary (the bug-triggering condition);
// "control" is the same structure shifted so the CRLF sits safely within one
// chunk, proving the bug is specifically about the boundary coincidence.
// Ground truth for both fixtures: 2 data rows (see generator script).
// ---------------------------------------------------------------------------

constexpr size_t kSyntheticBufSize = 64;
constexpr size_t kSyntheticBoundaryExpectedRows = 3;
constexpr size_t kSyntheticControlExpectedRows = 2;

TEST_CASE("[buggy] synthetic boundary file: in-place-reuse (expect off-by-one)")
{
    size_t rows = countDataRows(
        "tests/fixtures/boundary_64.csv.gz", kSyntheticBufSize, ZlibMiniReadBuffer::RefillStrategy::InPlaceReuse, skipRow);
...
{
    size_t rows = countDataRows(
        "tests/fixtures/control_64.csv.gz", kSyntheticBufSize, ZlibMiniReadBuffer::RefillStrategy::InPlaceReuse, skipRow);
...
{
    size_t rows = countDataRows(
        "tests/fixtures/boundary_64.csv.gz", kSyntheticBufSize, ZlibMiniReadBuffer::RefillStrategy::InPlaceReuse, skipRowFixed);
...
{
    size_t rows = countDataRows(
        "tests/fixtures/boundary_64.csv.gz", kSyntheticBufSize, ZlibMiniReadBuffer::RefillStrategy::FreeAndReallocate, skipRow);
...
{
    size_t rows = countDataRows(
        "tests/fixtures/boundary_64.csv.gz", kSyntheticBufSize, ZlibMiniReadBuffer::RefillStrategy::FreeAndReallocate, skipRowFixed);
